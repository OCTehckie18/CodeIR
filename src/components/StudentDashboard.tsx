...
export default function StudentDashboard({
  ...
  const handleAddSkill = async (e: React.FormEvent) => {
    ...
    if (newSkillName.trim() && user) {
      try {
        ...
        const res = await axios.post(`${apiUrl}/profiles/${user.id}/skills`, 
  ...
  const handleDeleteSkill = async (skillId: string) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await axios.delete(`${apiUrl}/skills/${skillId}`,
        { headers: { Authorization: `Bearer ${session?.access_token}` } }
      );
      if (res.data.success) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [profileResult, dashboardResult, skillsResult] = await Promise.allSettled([
        axios.get(`${apiUrl}/profiles/${session.user.id}`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }),
        axios.get(`${apiUrl}/dashboard/${session.user.id}`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }),
        axios.get(`${apiUrl}/profiles/${session.user.id}/skills`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }),
      ]);
  ...
  const calculateStats = (subs: any[]) => {
    ...
    const bestStreak = (() => {
      ...
      const uniqueDates = Array.from(
        new Set(
          subs.map(
            (s) => new Date(s.submission_timestamp).toISOString().split("T")[0],
          ),
        ),
      ).sort();
      let best = 1,
    ...
    const evaluated = subs.filter(
      (s) =>
        ...
        (Array.isArray(s.evaluations)
          ? s.evaluations[0]?.final_scores
          : s.evaluations?.final_scores),
    );
    const avgScores = { correctness: 0, efficiency: 0, style: 0 };
    if (evaluated.length > 0) {
      evaluated.forEach((s) => {
        const sc = Array.isArray(s.evaluations)
          ? s.evaluations[0]?.final_scores
          : s.evaluations?.final_scores;
        if (sc) {
      ...
      avgScores.correctness = Math.round(
        avgScores.correctness / evaluated.length,
      );
      avgScores.efficiency = Math.round(
        avgScores.efficiency / evaluated.length,
      );
                ...
                <div className="flex gap-4 items-center mb-6">
                  ...
                  <div className="flex flex-col z-10">
                    ...
                    <p
                      ...
                      style={{
                        color:
                          stats.totalSolved >= 20
                            ? "#a78bfa" // purple — Expert
                            : stats.totalSolved >= 10
                              ? "#60a5fa" // blue — Intermediate
                              : stats.totalSolved >= 5
                                ? "#34d399" // green — Apprentice
                                : "#22d3ee", // cyan — Novice
                      }}
                    >
                      Rank:{" "}
                    ...
                    {profile?.bio && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-[150px] line-clamp-2">
                        {profile.bio}
                      </p>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white/[0.02] backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg flex items-center gap-8 relative overflow-hidden group">
                  ...
                  <div className="relative w-28 h-28 flex-shrink-0">
                    ...
                    >
                      ...
                      <circle
                        ...
                        strokeDashoffset={
                          282.7 -
                          ((stats.totalSolved > 0
                            ? Math.min(stats.totalSolved * 10, 100)
                            : 0) /
                            100) *
                            282.7
                        }
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 uppercase font-bold tracking-widest mt-0.5">
                        Solved
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-4">
                    {[
                      {
                        label: "Correctness",
                        value: stats.avgScores.correctness,
                        color: "bg-emerald-500",
                      },
                      {
                        label: "Efficiency",
                        value: stats.avgScores.efficiency,
                        color: "bg-yellow-500",
                      },
                      {
                        label: "Code Style",
                        value: stats.avgScores.style,
                        color: "bg-rose-500",
                      },
                    ...
                    {stats.avgScores.correctness === 0 &&
                      stats.avgScores.efficiency === 0 && (
                        <p className="text-[11px] text-slate-500 dark:text-slate-500 italic">
                          Scores will appear after your first evaluated
                          submission.
                        </p>
                      )}
                  </div>
                ...
                <div className="bg-white/[0.02] backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg flex flex-col justify-center relative overflow-hidden group">
                  ...
                  <div className="flex items-baseline gap-2 z-10">
                    ...
                    <span className="text-slate-600 dark:text-slate-400 font-medium pb-1">
                      days
                    </span>
                  </div>
              ...
              <div className="bg-white/[0.02] backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg overflow-hidden flex flex-col">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                    ...
                    submissions{" "}
                    <span className="text-slate-500 dark:text-slate-500 font-normal hidden sm:inline">
                      in the past year
                    </span>
                  </h3>
                  ...
                  <table className="w-full text-left text-sm border-separate border-spacing-y-2">
                    <thead>
                      <tr className="text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wider font-semibold">
                        ...
                        <th className="px-4 py-2 text-right w-[100px]">
                          Status
                        </th>
                          ...
                          >
                            <td className="px-4 py-4 rounded-l-xl">
                              <div className="font-semibold text-slate-800 dark:text-slate-200 truncate pr-4">
                                {sub.problems?.problem_statement
                                  ? sub.problems.problem_statement.slice(
                                      0,
                                      45,
                                    ) +
                                    (sub.problems.problem_statement.length > 45
                                      ? "..."
                                      : "")
                                  : "Untitled Problem"}
                              </div>
                            ...
                            <td className="px-4 py-4">
                              {isEvaluated && evalObj.teacher_feedback ? (
                                <div
                                  className="text-[11px] text-slate-700 dark:text-slate-300 italic truncate max-w-[150px] lg:max-w-[200px]"
                                  title={evalObj.teacher_feedback}
                                >
